 ...
 * contributor license agreements.  See the NOTICE file dis.tributed with
...
object CS143Utils {
  ...
  def maybeSpill[K, V](collection: SizeTrackingAppendOnlyMap[K, V], allowedMemory: Long): Boolean = {
    collection.estimateSize() >= allowedMemory || collection.atGrowThreshold
  }
}
      ...
      def next() = {
        ...
        if(input.hasNext){
          ...
          if (!cache.containsKey(key_proj)) {
            cache.put(key_proj, udfProject(row))
          }
